...
def plot(signal, plot_type: str = None, show: bool = True, title: str = None, plot_kwargs=None, **kwargs):
    if signal.size < 2:
        kwargs['marker'] = kwargs.get('marker', 'o')  # put a single dot if there is only one data point

    is_complex = np.iscomplexobj(signal)
    if plot_type is None:
        plot_type = 'simple' if is_complex else 'bode'
    ptr = get_plotter(plot_type, complex_only=is_complex, **(plot_kwargs or {}))
